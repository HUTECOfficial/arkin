...
const ASESORES = [
  ...
  { id:'a2', nombre:'Roberto Silva', email:'roberto@arkin.mx', telefono:'+52 477 234 5678', especialidad:'Comercial e Industrial', propActivas:5, propVendidas:7, leads:31, visitas:18, ofertas:6, ventas:2, cartera:'$15,200,000', comisiones:'$304,000', rating:4.7, ultimaAct:'Hace 5 horas' },
  { id:'a3', nombre:'María López', email:'maria@arkin.mx', telefono:'+52 477 345 6789', especialidad:'Renta Residencial', propActivas:6, propVendidas:9, leads:27, visitas:15, ofertas:4, ventas:1, cartera:'$18,900,000', comisiones:'$378,000', rating:4.8, ultimaAct:'Hace 1 día' },
  { id:'a4', nombre:'Daniela Belmonte', email:'daniela@arkin.mx', telefono:'+52 477 456 7801', especialidad:'Desarrollos Nuevos', propActivas:4, propVendidas:5, leads:19, visitas:11, ofertas:3, ventas:1, cartera:'$9,800,000', comisiones:'$196,000', rating:4.6, ultimaAct:'Hace 3 horas' },
  { id:'a5', nombre:'Subje Hamue', email:'subje@arkin.mx', telefono:'+52 477 456 7802', especialidad:'Residencial Medio', propActivas:3, propVendidas:4, leads:14, visitas:8, ofertas:2, ventas:0, cartera:'$7,400,000', comisiones:'$148,000', rating:4.5, ultimaAct:'Hace 6 horas' },
]
const PROPIEDADES = [
...
export default function PanelEmpresaPage() {
  ...
  return (
    <div className="min-h-screen bg-arkin-dark">
      <header className="bg-arkin-dark text-white shadow-lg">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Badge className="bg-arkin-gold text-black text-xs px-2 py-0.5 hidden sm:flex items-center gap-1"><Award className="h-3 w-3"/> Team Elite</Badge>
            <Button variant="ghost" size="sm" onClick={()=>{logout();router.push('/')}} className="text-white hover:text-arkin-gold hover:bg-white/10 text-xs">
        ...
        {tab==='overview'&&<>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card className="border-0 shadow-sm">
              <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><Award className="h-4 w-4 text-arkin-gold"/>Ranking de Asesores</CardTitle></CardHeader>
              <CardContent className="space-y-1">
                {[...ASESORES].sort((a,b)=>b.ventas-a.ventas).map((a,i)=>(
                  <div key={a.id} onClick={()=>{setSelected(a);setTab('asesores')}} className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 cursor-pointer">
                    <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${i===0?'bg-arkin-gold text-black':i===1?'bg-gray-300 text-gray-700':i===2?'bg-amber-600 text-white':'bg-gray-100 text-gray-500'}`}>{i+1}</span>
                    <UserCircle className="h-7 w-7 text-arkin-primary flex-shrink-0"/>
            ...
            <Card className="border-0 shadow-sm">
              <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><Activity className="h-4 w-4 text-arkin-primary"/>Actividad Reciente</CardTitle></CardHeader>
                ...
                {ACTIVIDADES.slice(0,6).map(a=>(
                  <div key={a.id} className="flex items-start gap-3">
                    ...
                    <div><p className="text-sm text-gray-800 leading-snug">{a.descripcion}</p>
                      <div className="flex items-center gap-2 mt-0.5"><span className="text-xs text-arkin-primary font-medium">{a.asesor}</span><span className="text-xs text-gray-400">· {a.fecha}</span></div>
                    </div>
                  </div>
                ))}
              </CardContent>
          ...
          <Card className="border-0 shadow-sm">
            <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><TrendingUp className="h-4 w-4 text-arkin-primary"/>Embudo de Ventas del Equipo</CardTitle></CardHeader>
              ...
              <Card key={a.id} className="border-0 shadow-sm hover:shadow-md transition-shadow cursor-pointer" onClick={()=>setSelected(a)}>
                <CardContent className="p-5">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-12 h-12 rounded-full bg-arkin-primary/10 flex items-center justify-center flex-shrink-0"><UserCircle className="h-8 w-8 text-arkin-primary"/></div>
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-gray-900 truncate">{a.nombre}</p>
                      <p className="text-xs text-gray-500">{a.especialidad}</p>
                      <div className="flex items-center gap-0.5 mt-0.5">{[...Array(5)].map((_,i)=><Star key={i} className={`h-3 w-3 ${i<Math.floor(a.rating)?'text-arkin-gold fill-arkin-gold':'text-gray-200'}`}/>)}<span className="text-xs text-gray-500 ml-1">{a.rating}</span></div>
                    </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card className="border-0 shadow-sm">
              <CardContent className="p-6 text-center">
                <div className="w-20 h-20 rounded-full bg-arkin-primary/10 flex items-center justify-center mx-auto mb-4"><UserCircle className="h-12 w-12 text-arkin-primary"/></div>
                ...
                <div className="flex items-center justify-center gap-0.5 mb-4">{[...Array(5)].map((_,i)=><Star key={i} className={`h-4 w-4 ${i<Math.floor(selected.rating)?'text-arkin-gold fill-arkin-gold':'text-gray-200'}`}/>)}<span className="text-sm font-semibold ml-1">{selected.rating}</span></div>
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <Card className="border-0 shadow-sm"><CardContent className="p-5 space-y-3">
                ...
                <div className="flex items-center justify-between"><span className="text-sm font-semibold text-gray-700">Comisiones Generadas</span><span className="text-lg font-bold text-arkin-primary">{selected.comisiones}</span></div>
              </CardContent></Card>
              <Card className="border-0 shadow-sm">
              ...
              <Card key={p.id} className="border-0 shadow-sm hover:shadow-md transition-shadow">
                <CardContent className="p-5">
                  ...
                  <div className="flex items-center gap-2 pt-3 border-t">
                    <UserCircle className="h-4 w-4 text-arkin-primary flex-shrink-0"/>
                ...
                <tbody className="divide-y divide-gray-100">
                  {LEADS.map(l=>(
                    <tr key={l.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5"><UserCircle className="h-4 w-4 text-arkin-primary flex-shrink-0"/><span className="text-gray-700">{l.asesor}</span></div>
                      </td>
                  ...
                  <CardContent className="p-4 flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        <span className="text-xs text-arkin-primary font-semibold flex items-center gap-1"><UserCircle className="h-3 w-3"/>{a.asesor}</span>
              ...
              <Card className="border-0 shadow-sm">
                ...
                <CardContent>
                  {(()=>{
                    ...
                    return top ? (
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-arkin-gold/20 flex items-center justify-center"><UserCircle className="h-6 w-6 text-arkin-primary"/></div>
