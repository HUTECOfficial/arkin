          ...
          return (
            <button
              ...
              className={`relative p-1 text-xs rounded-lg transition-all ${
                isSelected
                  ? 'bg-arkin-gold text-black font-bold'
                  : isToday
        ...
        {([
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all whitespace-nowrap ${
              activeView === key
                ? 'bg-arkin-gold text-black shadow-lg'
                ...
                <CardContent className="p-0">
                  ...
                  <div className="p-5 space-y-4">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      ...
                      <div className="text-center p-3 rounded-xl bg-gray-50 dark:bg-gray-800">
                        <Home className="h-4 w-4 text-arkin-gold mx-auto mb-1" />
                      ...
                      <div className="w-full h-3 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all duration-700 ${
                            d.avanceObra >= 80 ? 'bg-green-500' : d.avanceObra >= 50 ? 'bg-arkin-gold' : 'bg-orange-500'
                          }`}
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="text-sm flex items-center gap-2">
                  <CalendarIcon className="h-4 w-4 text-arkin-gold" />
                  {formatDateDisplay(selectedDate)}
                  {selectedDate === getTodayStr() && <Badge className="bg-blue-100 text-blue-700 text-xs">Hoy</Badge>}
                </CardTitle>
                <Button size="sm" onClick={() => { setShowNewEvent(true); setNewEvent(prev => ({ ...prev, fecha: selectedDate })) }} className="bg-arkin-gold hover:bg-arkin-gold/90 text-black h-8 px-3 text-xs">
      ...
      {activeView === 'agente' && (
        <Card className="border-0 shadow-md overflow-hidden">
          <CardHeader className="bg-gradient-to-r from-slate-900 to-slate-800 text-white pb-3">
            <CardTitle className="text-sm flex items-center gap-2">
              <Bot className="h-4 w-4 text-arkin-gold" />
            ...
            <div className="h-[400px] overflow-y-auto p-4 space-y-3 bg-gray-50 dark:bg-gray-900">
              {chatMessages.map(msg => (
                <div key={msg.id} className={`flex gap-2.5 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  {msg.role === 'assistant' && (
                    <div className="w-7 h-7 rounded-full bg-arkin-gold/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                      <Bot className="h-3.5 w-3.5 text-arkin-gold" />
                    </div>
                  )}
                  <div className={`max-w-[80%] px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed ${
                    msg.role === 'user'
                      ? 'bg-arkin-gold text-black rounded-br-md'
            ...
            <div className="p-3 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
              <div className="flex gap-2">
                <Input
                  ...
                  className="text-sm"
                />
                <Button onClick={handleSendChat} className="bg-arkin-gold hover:bg-arkin-gold/90 text-black px-4 flex-shrink-0">
